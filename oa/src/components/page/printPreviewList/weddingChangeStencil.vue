<template>
    <div>
      <div class="wrap weddingChangeStencil" id="print">
			<p class="number">Bg<label id="agreementNo"></label></p>
			<p class="title">婚礼仪式及宴会服务变更协议书</p>
			...
			<p style="line-height:20px;">（下称“原协议”），现甲、乙双方就婚礼仪式及宴会服务变更一事达成如下协议，并共同遵照执行：</p>

			<ul>
				<li>一、甲方提出、乙方同意将婚礼举办日由原定的&nbsp;<label id="oldScheduleTime"  style="font-weight:bold"> </label>&nbsp;变更为 <label id="newScheduleTime" style="font-weight:bold"> </label></li>
				<li>二、婚礼举办场地由原定的&nbsp;<label id="oldBanquetHallName" style="font-weight:bold"> </label>&nbsp;变更为 <label id="newBanquetHallName" style="font-weight:bold"> </label></li>
...
<script type="text/ecmascript-6">
    ...
    export default {
        ...
        methods:{
            initData(){
                this.$axios.post('/order/getChangePrint',this.id).then((res)=> {  //获取所有套系
                    //订单详细信息
                   let orderVo =res.data.msg;
                    //合同详细信息
                    let agreementVo =orderVo["hsAgreementDTO"];
                    for(let i in agreementVo){
                        $("#"+i).text(agreementVo[i]);
                    }

                    $("#agreementNo").text(orderVo["agreementNo"]);
                    $("#partyB").text(orderVo["companyRegistrationName"]);
                   let change = orderVo.hsAgreementChangeDTO;
                    let createTime = agreementVo["createTime"];
                    ...
                    $("#date").text(date1);
                    if(change){
                        let totalAmount =agreementVo["totalAmount"];
                        let changeAmount =change["changeAmount"];
                        let oldSchedule =change["oldScheduleList"];
                        $("#oldScheduleTime").text(oldSchedule[0]["scheduleTime"].substr(0,10));
                        let oldBanquetHallName ="";
                        for(let i in oldSchedule){
                        ...
                        let newSchedule =change["newScheduleList"];
                        let newBanquetHallName ="";
