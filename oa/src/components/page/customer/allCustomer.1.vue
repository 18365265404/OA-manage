    ...
    export default {
        ...
        methods: {
			selectExcel(){
				...
				if(startTime && endTime && endTime-startTime<=200518400000 && endTime-startTime>=0){
			...
			list.map(item=>{
					...
					switch (item.cityCode) {
						case '110100':
							...
							item.cityName='北京市'
							this.excelList[0].allTimes ++;
							this.getStatus(item.status,0)
							break;

						case '350200':
							item.cityName='厦门市'
							this.excelList[1].allTimes ++;
							this.getStatus(item.status,1)
							break;

						case '440600':
							item.cityName='佛山市'
							this.excelList[2].allTimes ++;
							this.getStatus(item.status,2)
							break;

						case '330200':
							item.cityName='宁波市'
							this.excelList[3].allTimes ++;
							this.getStatus(item.status,3)
							break;
							
						case '330300':
							item.cityName='温州市'
							this.excelList[4].allTimes ++;
							this.getStatus(item.status,4)
							break;
						
						case '330500':
							item.cityName='湖州市'
							this.excelList[5].allTimes ++;
							this.getStatus(item.status,5)
							break;

						case '330600':
							item.cityName='绍兴市'
							this.excelList[6].allTimes ++;
				...
				this.tableExcelList=this.excelList
