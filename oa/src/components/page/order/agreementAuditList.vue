       ...
       <el-form :inline="true" :model="formInline" ref="formInline" class="demo-form-inline grayForm">
		  ...
		  <el-form-item label="到" prop='dateTimeStop'>
			...
			<el-form-item  prop='orderType'>
	            <el-select clearable v-model="formInline.orderType" placeholder="订单类型">
	                <el-option
	        ...
	        <el-form-item  prop='applyItemType'>
	            <el-select clearable v-model="formInline.applyItemType" placeholder="审核状态">
	                <el-option
		    ...
		    <el-form-item>
			   <el-button type="primary" @click="resetForm('formInline')">重置</el-button>
	            ...
	            <el-table :data="duplicateDataList" border style="width: 100%" v-loading="loading.pageList" element-loading-text="拼命加载中" >
	                ...
	                <el-table-column prop="orderTypeText" label="类型">
	                ...
	                <el-table-column prop="banquetHallName" label="宴会厅"></el-table-column>
	                <el-table-column prop="agreementStatus" label="合同状态">
	                	<template scope="scope">
			                <div>{{agreementStatusText(scope.row.agreementStatus)}}</div>
			            </template>
	                </el-table-column>
	                <el-table-column prop="agreementCreateTime" label="合同日期">
                        <template scope="scope">
                            {{formTime1(scope.row.agreementCreateTime)}}
                        </template>
                    </el-table-column>
	                <el-table-column label="合同详情" v-if='btnP.agreementAudShowDetails'>
	                	<template scope="scope">
		                    <el-button size="mini" type="primary" @click="checkDetailsRow(scope.row)">合同详情</el-button>
		                </template>
	                </el-table-column>
	                <el-table-column prop="" label="财务详情" v-if='btnP.agreementAudShowMoneyDetails'>
                        <template scope="scope">
                            <el-button size="mini" type="primary" @click="rowFinanceDetail(scope.row)">财务详情</el-button>
                        </template>
	                </el-table-column>
	                <el-table-column prop="applyItemType" label="审核项目">
	                	<template scope="scope">
			                <div>{{typeText(scope.row.applyItemType)}}</div>
			            </template>
	                </el-table-column>
	                <el-table-column label="审核操作" v-if='btnP.agreementAudExamine'>
	                	<template scope="scope">
		                    <el-button size="mini" type="primary" @click="applyItemTypeHandle(scope.row)">审核</el-button>
		                </template>
	                </el-table-column>
        ...
        <el-dialog title="审核合同-合同签订" :visible.sync="applyItemTypeVasible" :before-close='cancelapplyItemTypeHandle'>
			  <el-form :model="applyItemTypeData" ref='applyItemTypeData' :rules='applyItemTypeDataRules'>
			    <el-form-item prop='remark' :label-width="formLabelWidth">
			      <el-input type='textarea' placeholder='请输入审核结果' v-model="applyItemTypeData.remark" auto-complete="off"></el-input>
			    </el-form-item>

			  </el-form>
			  <div slot="footer" class="dialog-footer">
			    <el-button type="primary" icon='document' @click="sureapplyItemTypeHandle('applyItemTypeData')">确定</el-button>
			    <el-button @click="cancelapplyItemTypeHandle">取 消</el-button>
			  </div>
			</el-dialog>
    ...
    export default {
        data() {
            return {
                ...
                btmVisible:false,
                btnP:{
                	agreementAudShowDetails:"", //查看详情
		            agreementAudShowMoneyDetails:"", //查看财务详情
		            agreementAudExamine:"", //审核
                }
            }
        },
        ...
        methods: {
			...
			getAllDuplicateDataList(data){
				...
				this.$axios.post('/order/getAgreementAuditList',data).then((res)=>{
                   
					if(res.data.res == 0){
                        
                        let centerArr=[]
                        
						this.dec.review_type.forEach((n)=>{
			                if(n.dicValue == 2 || n.dicValue == 3){
			                	centerArr.push(n)
			                }
                        })
                        this.reviewTypes=this.uniqueArray(centerArr,"dicValue")
			...
			applyItemTypeHandle(row){  //审核合同
				this.applyItemTypeVasible = true
				this.clickApplyItemTypeRow = row
			},
			cancelapplyItemTypeHandle(){
				this.$refs['applyItemTypeData'].resetFields();
 				this.applyItemTypeVasible = false
			},
			sureapplyItemTypeHandle(formName){  //确定审核合同

				let data = {
					"hsOrderId":this.clickApplyItemTypeRow.hsOrderId,
					"remark":this.applyItemTypeData.remark,
					"type":this.clickApplyItemTypeRow.applyItemType
				}
				this.$refs[formName].validate((valid) => {
		          if (valid) {
	            	this.$axios.post('/order/reviewAgreement',data).then((res)=>{
	                    if(res.data.res==0){
	                    	this.$message({
					            type: 'success',
					            message: '审核成功!'
	                        ...
	                        this.getAllDuplicateDataList(this.tableData)
	                        this.cancelapplyItemTypeHandle()
	                    }else {
        ...
        mounted(){
        	this.btnP = {
                	agreementAudShowDetails:filterBtnPms(this.btnAllPms.agreementAudShowDetails), //查看详情
		            agreementAudShowMoneyDetails:filterBtnPms(this.btnAllPms.agreementAudShowMoneyDetails), //查看财务详情
		            agreementAudExamine:filterBtnPms(this.btnAllPms.agreementAudExamine), //审核
                }
            this.initDec()
        ...
        computed:{
            tableData(){
                return  {"pageNow":this.pagination.pageNow, "pageSize":this.pagination.pageSize,"salesId":this.formInline.salesId,"dateTimeStart":this.formInline.dateTimeStart,"dateTimeStop":this.formInline.dateTimeStop,"orderType":this.formInline.orderType,"applyItemType":this.formInline.applyItemType,"cityCode":this.cityNames.city}
            },
            ...mapGetters([
                'dicData',
                'allPermission',
