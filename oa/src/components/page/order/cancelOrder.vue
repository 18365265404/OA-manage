    ...
    <div>
        <div class="crumbs">
            <el-breadcrumb separator="/">
                ...
                <el-breadcrumb-item>合同取消变更申请</el-breadcrumb-item>
            </el-breadcrumb>
            </div>
        <div>
        	<el-form :inline="true" :model="formInline" ref="formInline" class="demo-form-inline grayForm">
	       	<el-form-item label="申请日期" prop='dateTimeStart'>
			     <div class="block">
				    <el-date-picker
	          ...
	          <el-form-item prop='mobile'>
	            ...
	            <el-form-item  prop="applyItemType">
	                <el-select clearable v-model="formInline.applyItemType" placeholder="申请项目" size="small">
	                    <el-option
	                        v-for="item in dec.review_type"
	                        :key="item.dicDesc"
	                        :label="item.dicDesc"
	                        :value="item.dicValue">
	                    </el-option>
        ...
        <el-table :data="cancelAndChangeList" border style="width: 100%">
            ...
            <el-table-column prop="applyTime" label="申请时间" width="180px" :formatter="formTime">
            </el-table-column>
            <el-table-column prop="scheduleTime" label="档期" width="150px" >
            </el-table-column>
            <el-table-column prop="scheduleTimeType" label="场次" width="150px" >
                <template scope="scope">
                    {{statusText(scope.row.scheduleTimeType,'schedule_time_type')}}
                </template>
            </el-table-column>
            <el-table-column prop="banquetHallName" label="宴会厅" width="150px">
            </el-table-column>
            <el-table-column prop="agreementTime" label="合同日期" width="180px" :formatter="formTime1">
            </el-table-column>
            ...
            <el-table-column prop="applyItemType" label="申请类型" >
            	<template scope="scope">
                    {{statusText(scope.row.applyItemType,'review_type')}}
                </template>
            </el-table-column>
            <el-table-column label="申请详情" v-if='btnP.cancelOrderApplyReview'>
                <template scope="scope">
					<el-button size="mini" v-show='scope.row.applyItemType == 0' type="primary" @click="lookDetailsRow(scope.row)">查看取消合同</el-button>
					<el-button size="mini" v-show='scope.row.applyItemType == 1' type="primary" @click="lookDetailsRow1(scope.row)">查看变更合同</el-button>
                </template>
            </el-table-column>
        </el-table>
        ...
        <el-dialog :title="hsOrderDialogTitle" size='large' :visible.sync="dialogNewContract" :before-close='closeDialogNewContract'>
 		...
 		<el-dialog size='large'  title="申请取消合同" :visible.sync="applyCancelOrderVisible" :before-close='cancelApplyCancelOrderHandle'>
 			<el-form :model="cancelOrderDetailList">
	    		<div>
	    			<el-form-item class='inline-form' prop='orderNo' label="订单号：" :label-width="hetongDetailsWidth1">
	                    <span class='hiddenspan3'>{{cancelOrderDetailList.orderNo}}</span>
	                </el-form-item >
					<el-form-item class='inline-form' prop='orderNo' label="已付：" :label-width="hetongDetailsWidth1">
		                <span class='hiddenspan3'>{{cancelOrderDetailList.paidAmount}}</span>
		           </el-form-item >
				 	<el-form-item class='inline-form' prop='orderNo' label="确认已付：" :label-width="hetongDetailsWidth1">
		                <span class='hiddenspan3'>{{cancelOrderDetailList.confirmPaidAmount}}</span>
		            </el-form-item >
	    		</div>
				<div>
					<el-form-item class='inline-form' prop='orderNo' label="申请人：" :label-width="hetongDetailsWidth1">
		                <span class='hiddenspan3'>{{cancelOrderDetailList.creatorName}}</span>
		            </el-form-item >
				</div>
				<div>
					<el-form-item class='inline-form' prop='orderNo' label="申请处理意见：" :label-width="hetongDetailsWidth1">
		                <span class='hiddenspan3'>{{cancelOrderDetailList.remark}}</span>
		            </el-form-item >
				</div>
				<div>
					<el-form-item class='inline-form' prop='orderNo' label="申请退款：" :label-width="hetongDetailsWidth1">
		                <span class='hiddenspan3'>{{cancelOrderDetailList.refundsAmount}}</span>
		            </el-form-item >
				</div>
			 </el-form>
			 <div style='height:36px;line-height:36px;'>
			 	<el-button type="primary" @click='agreeAapplyHandle'>通过</el-button>
			 	<el-button type="primary" @click='noneAgreeApplyHandle'>不通过</el-button>
			 </div>
			 <div v-show='isChangeRefund' v-if='btnP.cancelOrderChangeMoney'>
			 	<div style='margin-top:30px;padding-left:30px;margin-bottom:30px'>
				 	<el-switch
	                    v-model="isNeedGiveMoney"
	                    on-text="需要退款"
	                    off-text="不需要退款"
	                    :width="100">
	                </el-switch>
				</div>
				<el-input style='width:50%' v-show='isNeedGiveMoney' v-model="refundAmount" placeholder="填写退款金额"></el-input>
			 </div>
			  <div style='height:36px;line-height:36px;margin-top:30px'>
			 	<el-button type="primary" v-show='!isChangeRefund && btnP.cancelOrderChangeMoney' @click='modifyRefundHandle'>修改退款</el-button>
			 	<el-button type="primary" v-show='isChangeRefund && btnP.cancelOrderChangeMoney' @click='confirmModifyRefundHandle'>确定</el-button>
			 	<el-button type="primary" @click='cancelApplyCancelOrderHandle'>取消</el-button>
			 </div>
        </el-dialog>
        <el-dialog size='large'  title="申请变更合同" :visible.sync="applyCancelOrderVisible1" :before-close='cancelApplyCancelOrderHandle1'>
        	<p>原合同信息</p>
 			 <el-table :data='oldOrderTableData' border style="width: 100%">
            ...
            <p class='marginTop'>变更信息</p>
            <el-table :data='newOrderTableData' border style="width: 100%">
                <el-table-column prop="scheduleTime" label="婚礼日"></el-table-column>
                <el-table-column prop="storeName" label="门店"></el-table-column>
                <el-table-column prop="hallName" label="宴会厅"></el-table-column>
                <el-table-column prop="scheduleTimeType" label="场次">
			 ...
			 <div class='marginTop'>
			  ...
			  <div class='marginTop'>
			 	<el-button type="primary" @click='agreeAapplyHandle1'>通过</el-button>
			 	<el-button type="primary" @click='noneAgreeApplyHandle1'>不通过</el-button>
			 	<el-button type="primary" @click='cancelApplyCancelOrderHandle1'>取消</el-button>
			 </div>
        </el-dialog>
...
<script type="text/ecmascript-6">
    ...
    export default{
     ...
     methods: {
     	...
     	onSubmit(){
			this.initCancelAndChangeList(this.tableData)
        },
        resetForm(formName){
        ...
        formTime1(row){
            return formatTime(row.agreementTime)
        },
         startDateChange(val){
         ...
         initCancelAndChangeList(data){
             this.$axios.post('/order/getCancelAndChangeList',data).then((res)=> {
                 if (res.data.res == '0') {
                     ...
                     this.cancelAndChangeList=res.data.msg.list;
                 }
             })
         },
         initDec(){ //初始化字典值
         ...
         getAllBanquets(){
            	...
            	this.$axios.get('/store/getHallList',{params:{'pageSize':100}}).then((res)=>{  //获取所有宴会厅
                    if(res.data.res == '0'){
			...
			lookDetailsRow1(row){  //查看申请详情(变更)
				// console.log("11111111111111111111111111",row.hsOrderId)
				// alert(row.hsOrderId)
				// return
            	this.cancelOrderDetailListRow = row
            	this.applyCancelOrderVisible1 = true
            	this.$axios.post('/order/getAgreementChange',{hsOrderId:row.hsOrderId}).then((res)=>{
					console.log("2222222222222222",res.data)
		            if(res.data.res==0){
		                this.oldOrderTableData = res.data.msg.oldScheduleList
		                this.newOrderTableData = res.data.msg.newScheduleList
		                this.changeAmount = res.data.msg.changeAmount
            			this.changeReason = res.data.msg.remark
		            }else {
		                alert(res.data.msg)
		            }
		        })
            ...
            agreeAapplyHandle1(){
            	this.$confirm('是否确认通过 ?', '信息', {
                    confirmButtonText: '确定',
                    cancelButtonText: '取消',
                    type: 'warning'
                }).then(() => {
                    this.$axios.post('/order/reviewAgreementChange',{id:this.cancelOrderDetailListRow.id,hsOrderId:this.cancelOrderDetailListRow.hsOrderId,reviewStatus:1}).then((res)=>{
                        if(res.data.res==0){
                   			this.initCancelAndChangeList(this.tableData)
                            this.cancelApplyCancelOrderHandle1()
                            this.$message({
                                type: 'success',
                                message: '审核通过!'
                            });
            ...
            noneAgreeApplyHandle1(){
            	this.$confirm('是否确认不通过 ?', '信息', {
                    confirmButtonText: '确定',
                    cancelButtonText: '取消',
                    type: 'warning'
                }).then(() => {
                    this.$axios.post('/order/reviewAgreementChange',{id:this.cancelOrderDetailListRow.id,hsOrderId:this.cancelOrderDetailListRow.hsOrderId,reviewStatus:2}).then((res)=>{
                        if(res.data.res==0){
                   			this.initCancelAndChangeList(this.tableData)
                            this.cancelApplyCancelOrderHandle1()
                            this.$message({
            ...
            cancelApplyCancelOrderHandle1(){  //取消申请详情(变更)
            	this.applyCancelOrderVisible1 = false
            },
            lookDetailsRow(row){  //查看申请详情(取消)
            	this.applyCancelOrderVisible = true
            	this.cancelOrderDetailListRow = row
            	this.isChangeRefund = false
            	this.$axios.post('/order/getAgreementCancel',{hsOrderId:row.hsOrderId}).then((res)=>{
		            if(res.data.res==0){
		                this.cancelOrderDetailList = res.data.msg
		                if(this.cancelOrderDetailList.isRefunds == 1){
		                	this.cancelOrderDetailList.refundsAmount = this.cancelOrderDetailList.refundsAmount
		                }
		                else{
		                	this.cancelOrderDetailList.refundsAmount = '不需要退款'
		                }
		            }else {
		                alert(res.data.msg)
		            }
            ...
            cancelApplyCancelOrderHandle(){
            	this.applyCancelOrderVisible = false
            },
			modifyRefundHandle(){
				this.isChangeRefund = true
			},
			confirmModifyRefundHandle(){  //确定申请退款
				if(this.isNeedGiveMoney){
					if(this.refundAmount === ''){
						this.$message.error('请填写退款金额！')
						return
					}
					else{
						this.$confirm('是否确认修改退款?', '信息', {
		                    confirmButtonText: '确定',
		                    cancelButtonText: '取消',
		                    type: 'warning'
		                }).then(() => {
		                    this.$axios.post('/order/updateRefundsAmount',{id:this.cancelOrderDetailListRow.id,hsOrderId:this.cancelOrderDetailListRow.hsOrderId,isRefunds:1,refundsAmount:this.refundAmount}).then((res)=>{
		                        if(res.data.res==0){
		                   			this.initCancelAndChangeList(this.tableData)
		                   			this.cancelOrderDetailList.refundsAmount = this.refundAmount
		                   			this.applyCancelOrderVisible = true
					            	this.isChangeRefund = false
		                            this.$message({
		                                type: 'success',
					...
					this.$confirm('是否确认修改退款? ?', '信息', {
		                ...
		                this.isChangeRefund = false
		                this.$axios.post('/order/updateRefundsAmount',{id:this.cancelOrderDetailListRow.id,hsOrderId:this.cancelOrderDetailListRow.hsOrderId,isRefunds:0}).then((res)=>{
		                    if(res.data.res==0){
		                   		this.initCancelAndChangeList(this.tableData)
		                   		this.cancelOrderDetailList.refundsAmount = '不需要退款'
		                   		this.refundAmount = ''
            ...
            agreeAapplyHandle(){  //通过取消合同
            	this.$confirm('是否确认通过 ?', '信息', {
                    confirmButtonText: '确定',
                    cancelButtonText: '取消',
                    type: 'warning'
                }).then(() => {
                    this.$axios.post('/order/reviewAgreementCancel',{id:this.cancelOrderDetailListRow.id,hsOrderId:this.cancelOrderDetailListRow.hsOrderId,reviewStatus:1}).then((res)=>{
                        if(res.data.res==0){
                   			this.initCancelAndChangeList(this.tableData)
                            this.cancelApplyCancelOrderHandle()
                            this.$message({
                                type: 'success',
                                message: '审核通过!'
                            });
            ...
            noneAgreeApplyHandle(){  //不通过取消合同
            	this.$confirm('是否确认不通过 ?', '信息', {
                    confirmButtonText: '确定',
                    cancelButtonText: '取消',
                    type: 'warning'
                }).then(() => {
                    this.$axios.post('/order/reviewAgreementCancel',{id:this.cancelOrderDetailListRow.id,hsOrderId:this.cancelOrderDetailListRow.hsOrderId,reviewStatus:2}).then((res)=>{
                        if(res.data.res==0){
                   			this.initCancelAndChangeList(this.tableData)
                            this.cancelApplyCancelOrderHandle()
                            this.$message({
    ...
    computed:{
        tableData(){
            return {
                ...
                "applyItemType":this.formInline.applyItemType,
