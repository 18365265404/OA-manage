<template>
    <div>
        <el-breadcrumb separator="/">
            ...
            <el-breadcrumb-item>待分配合同订单</el-breadcrumb-item>
        </el-breadcrumb>
        <div>
            <el-form :inline="true" :model="researchNoAssignedOrder" ref="researchNoAssignedOrder" class="demo-form-inline">
                <el-row type="flex" justify="center" style="width: 90%;margin: 20px auto" >
                    <el-col :span="9">
                        <el-form-item  prop='mobile' :label-width="researchNoAssignedOrderWidth">
                            <el-input  v-model="researchNoAssignedOrder.mobile" placeholder="客户电话" size="small"></el-input>
                        </el-form-item>
                        <el-form-item :label-width="researchNoAssignedOrderWidth">
                            <el-select @change='changeCityHandle' v-model="researchNoAssignedOrder.cityCode" placeholder="城市" size="small">
                                <el-option
                        ...
                        <el-form-item  prop='dateTimeStart' :label-width="researchNoAssignedOrderWidth">
                            <div class="block">
                                <el-date-picker clearable
                                    v-model="researchNoAssignedOrder.dateTimeStart"
                                    type="date"
                                    :editable="false"
                                    placeholder="档期起始日期"
                                    format="yyyy-MM-dd"
                        ...
                        <el-form-item  prop='dateTimeStop' :label-width="researchNoAssignedOrderWidth">
                            <el-tooltip class="item" effect="dark" content="终止日期为60天以内" placement="top-end">
                                <div class="block">
                                    <el-date-picker
                                        clearable
                                        v-model="researchNoAssignedOrder.dateTimeStop"
                                        type="date"
                                        :editable="false"
                    ...
                    <el-col :span="10">
                        <el-form-item  prop="orderType">
                            <el-select clearable v-model="researchNoAssignedOrder.orderType" placeholder="订单类型" size="small">
                                <el-option
                        ...
                        <el-form-item  prop="orderStatus">
                            <el-select clearable v-model="researchNoAssignedOrder.orderStatus" placeholder="订单状态" size="small">
                                <el-option
                        ...
                        <el-form-item  prop="agreementStatus">
                            <el-select clearable v-model="researchNoAssignedOrder.agreementStatus" placeholder="合同状态" size="small">
                                <el-option
                        ...
                        <el-form-item :label-width="researchNoAssignedOrderWidth" prop="hallId">
                            <el-select clearable size="small" v-model="researchNoAssignedOrder.hallId" placeholder="宴会厅">
                        ...
                        <el-form-item :label-width="researchNoAssignedOrderWidth" prop="salesId">
                            <el-select clearable size="small" v-model="researchNoAssignedOrder.salesId" placeholder="销售">
                                <el-option
                    ...
                    <el-col :span="5">
                        <el-form-item>
                            <el-button size="small" type="primary" @click="submitNoAssignedOrder">查询</el-button>
                        </el-form-item>
                        <el-form-item>
                            <el-button  size="small" type="primary" @click="resetForm('researchNoAssignedOrder')">重置</el-button>
                        </el-form-item>
                    </el-col>
                </el-row>
        ...
        <el-table v-loading="loading.pageList" element-loading-text="拼命加载中" :data="NoAssignedOrderTable" border style="width: 100%">
            ...
            <el-table-column prop="creatorName" label="销售"  >
            </el-table-column>
            ...
            <el-table-column prop="agreementStatus" label="合同状态" >
                <template scope="scope">
                    {{statusText(scope.row.agreementStatus,'agreement_status')}}
                </template>
            </el-table-column>
            <el-table-column prop="customerName" label="合同详情" v-if='btnP.notAssignedShowDetails'>
                <template scope="scope">
					<el-button  size="mini" type="primary" @click="checkDetailsRow(scope.row)">查看详情</el-button>
                </template>
            </el-table-column>
            <el-table-column  label="策划师" v-if='btnP.notAssignedAssign'>
                <template scope="scope">
                    <el-button size="small" type="primary" @click="assignPlanner(scope.row)">分配</el-button>
                </template>
            </el-table-column>
        </el-table>
    ...
    export default{
        data(){
            return {
                ...
                openCity:[],
                salesList:[],
                researchNoAssignedOrderWidth:'80px',
                beforeTime:'',
                ...
                pickerBeginDateAfter:{
                    disabledDate:(time)=>{
                        let beginDateVal = this.researchNoAssignedOrder.dateTimeStart;
                        if (beginDateVal) {
                ...
                btmVisible:false,
                btnP:{
                	notAssignedAssign:"", //分配（已有）
           			notAssignedShowDetails:"", //查看详情
                }
        ...
        methods: {
                ...
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        this.$axios.post('/order/assignPlanner',data).then((res)=>{
                            if(res.data.res == '0'){
                                this.initNoAssignedOrder(this.tableData)
            ...
            submitNoAssignedOrder(){
                this.researchNoAssignedOrder.dateTimeStart=this.researchNoAssignedOrder.dateTimeStart?moment(this.researchNoAssignedOrder.dateTimeStart).format('YYYY-MM-DD'):''
                this.researchNoAssignedOrder.dateTimeStop=this.researchNoAssignedOrder.dateTimeStop?moment(this.researchNoAssignedOrder.dateTimeStop).format('YYYY-MM-DD'):''
                if(getDays(this.researchNoAssignedOrder.dateTimeStart,this.researchNoAssignedOrder.dateTimeStop)>60){
                    this.$notify.error({
                        title: '错误',
            ...
            dateTimeStartChange(val){
                this.researchNoAssignedOrder.dateTimeStart = val
            },
            dateTimeStopChange(val){
                this.researchNoAssignedOrder.dateTimeStop = val
            },
            ...
            initNoAssignedOrder(data){
                this.loading.pageList=true
                this.$axios.post('/order/getNotAssignedHsOrderList',data).then((res)=> {
                    if (res.data.res == '0') {
                        ...
                        this.NoAssignedOrderTable=res.data.msg.list;
                        this.initDec()
            ...
            initCity(){ //初始化默认用户城市
                ...
                this.researchNoAssignedOrder.cityCode=this.storeCitys[0].cityCode
            },
            initOpenCity(){ //所有开通城市
            ...
            initPlannerList(){
                this.$axios.post('/s/getPlannerList',{cityCode:this.researchNoAssignedOrder.cityCode}).then((res2)=>{
                    if(res2.status == 200){
            ...
            getAllBanquets(){
            	...
            	this.$axios.get('/store/getHallList',{params:{'pageSize':100,cityCode:this.researchNoAssignedOrder.cityCode,'status':0}}).then((res)=>{  //获取所有宴会厅
                    if(res.data.res == '0'){
        ...
        computed:{
            tableData(){
                return {
                    ...
                    'dateTimeStart':this.researchNoAssignedOrder.dateTimeStart,
                    'dateTimeStop':this.researchNoAssignedOrder.dateTimeStop,
                    'mobile':this.researchNoAssignedOrder.mobile,
                    'hallId':this.researchNoAssignedOrder.hallId,
                    'orderType':this.researchNoAssignedOrder.orderType,
                    'cityCode':this.researchNoAssignedOrder.cityCode,
                    'salesId':this.researchNoAssignedOrder.salesId,
                    'agreementStatus':this.researchNoAssignedOrder.agreementStatus,
                    'orderStatus':this.researchNoAssignedOrder.orderStatus,
                }
        ...
        mounted(){
        	this.btnP = {
                	notAssignedAssign:filterBtnPms(this.btnAllPms.notAssignedAssign), //分配（已有）
           			notAssignedShowDetails:filterBtnPms(this.btnAllPms.notAssignedShowDetails), //查看详情
                }
            this.initSalesList()
            this.getAllBanquets()
            this.initCity()
